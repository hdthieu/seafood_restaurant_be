...
export class OrdersService {
  ...
  async create(dto: CreateOrderDto) {
    return this.ds.transaction(async (em) => {
      ...
      // load menu items
      const menuItems = await em.getRepository(MenuItem).find({
      ...
      // create items
      const items = dto.items.map(i => em.getRepository(OrderItem).create({
        order,
...
async list(params: { page: number; limit: number; status?: OrderStatus; excludeStatus?: string }) {
  ...
  /** UPDATE STATUS (with transition & inventory) */
  async updateStatus(orderId: string, dto: UpdateOrderStatusDto /*, userId?: string*/) {
    return this.ds.transaction(async (em) => {
      ...
      if (to === OrderStatus.CONFIRMED) {
        await this.consumeInventoryForOrder(em, order);
      }
      // hoàn kho nếu cancel sau khi đã confirm trở đi (chưa paid)
      if (to === OrderStatus.CANCELLED && [OrderStatus.CONFIRMED, OrderStatus.PREPARING, OrderStatus.READY, OrderStatus.SERVED].includes(from)) {
  ...
  /** ADD ITEMS (only PENDING/CONFIRMED) */
  async addItems(orderId: string, dto: AddItemsDto) {
    return this.ds.transaction(async (em) => {
      ...
      // upsert into order_items (gộp qty nếu đã có)
      ...
      // nếu đơn đã CONFIRMED thì phải trừ kho ngay phần tăng thêm
  ...
  /** REMOVE 1 item (only PENDING/CONFIRMED) */
  async removeItem(orderId: string, orderItemId: string) {
    return this.ds.transaction(async (em) => {
      ...
      // nếu đã CONFIRMED thì hoàn kho phần bị bớt
      if (order.status === OrderStatus.CONFIRMED) {
  ...
  // Trừ kho cho toàn bộ đơn theo công thức Ingredient * qty
  private async consumeInventoryForOrder(em: any, order: Order) {
    // load ingredients for all menu items in order
  ...
  // Khi bổ sung món vào đơn đã CONFIRMED
  private async consumeInventoryForDelta(em: any, orderId: string, deltas: { menuItemId: string; quantity: number; }[]) {
    const ingredients = await em.getRepository(Ingredient).find({
  ...
  // Hoàn kho toàn bộ đơn (khi CANCELLED sau CONFIRMED/…)
  private async restoreInventoryForOrder(em: any, order: Order) {
    const menuIds = order.items.map(it => it.menuItem.id);
    const ingredients = await em.getRepository(Ingredient).find({
      where: { menuItem: { id: In(menuIds) } },
      relations: ['inventoryItem', 'menuItem'],
  ...
  // Hoàn kho phần delta khi bớt món lúc CONFIRMED
  private async restoreInventoryForDelta(em: any, deltas: { menuItemId: string; quantity: number; }[], orderId: string) {
    const ingredients = await em.getRepository(Ingredient).find({
      where: { menuItem: { id: In(deltas.map(d => d.menuItemId)) } },
      relations: ['inventoryItem', 'menuItem'],
    });
  ...
  // Áp delta kho (IN/OUT) + ghi InventoryTransaction; validate tồn khi OUT
...
async setItemQty(orderId: string, orderItemId: string, quantity: number) {
  return this.ds.transaction(async (em) => {
    ...
    // tính delta để xử lý kho nếu đã CONFIRMED
    const delta = quantity - row.quantity;

    if (quantity <= 0) {
      await itRepo.delete(row.id);
      if (order.status === OrderStatus.CONFIRMED && row.quantity > 0) {
        await this.restoreInventoryForDelta(em, [
      ...
      if (order.status === OrderStatus.CONFIRMED && delta !== 0) {
        if (delta > 0) {
          await this.consumeInventoryForDelta(em, order.id, [
...
async cancel(orderId: string, dto: CancelOrderDto) {
    return this.ds.transaction(async (em) => {
      ...
      // 1) Không cho hủy nếu đã thanh toán
      if (order.status === OrderStatus.PAID) {
        throw new BadRequestException('ORDER_ALREADY_PAID');
      }

      // 2) Nếu có invoice: xử lý trước
      const inv = await iRepo.findOne({ where: { order: { id: orderId } } });
      if (inv) {
        if (inv.status === InvoiceStatus.PAID) {
          throw new BadRequestException('INVOICE_ALREADY_PAID');
        }
        // mark CANCELLED/VOID (tuỳ enum, dùng CANCELLED nếu đã có)
        inv.status =
          (InvoiceStatus as any).CANCELLED ?? (InvoiceStatus as any).VOID ?? InvoiceStatus.UNPAID;
        await iRepo.save(inv);
      }

      // 3) Chuyển trạng thái đơn -> CANCELLED
      //    Dùng lại transition + hoàn kho sẵn có trong updateStatus
      const updated = await this.updateStatus(orderId, { status: OrderStatus.CANCELLED });

      // 4) (tuỳ chọn) emit socket để kitchen/cashier làm mới
      // this.gateway.emit('order:cancelled', { orderId, tableId: order.table.id, reason: dto.reason });
